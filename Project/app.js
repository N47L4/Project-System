...
function updateBrowsePageUi() {
    ...
    if (currentUserLoggedIn) {
        ...
        if (primaryAction) {
            ...
            primaryAction.href = "browse.html";
        }
        if (secondaryAction) {
...
function fetchPendingBorrows() {
    fetch("/pending-borrows")
        .then((response) => {
            if (!response.ok) throw new Error("Failed to fetch pending borrows");
            return response.json();
        })
        .then((data) => {
            renderPendingBorrows(data);
        })
        .catch((error) => {
...
function renderPendingBorrows(borrows) {
    const container = document.getElementById("pending-borrows");
    if (!container) return;

    if (!borrows || borrows.length === 0) {
        container.innerHTML = '<article class="card"><p>No pending borrow requests.</p></article>';
        return;
    }
        ...
        .map(
            (borrow) => `
                <article class="audit-card">
                    <p><strong>Book ID:</strong> ${borrow.bookId} - <strong>User:</strong> ${borrow.username}</p>
                    <div style="display: flex; gap: 10px; margin-top: 10px;">
                        <button class="button" type="button" onclick="approveBorrow('${borrow.bookId}', 'approve')">Approve</button>
                        <button class="button secondary" type="button" onclick="rejectBorrow('${borrow.bookId}', 'reject')">Reject</button>
                    </div>
                </article>
            `
        )
...
function approveBorrow(bookId, action) {
    fetch("/approve-borrow", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: `book=${encodeURIComponent(bookId)}&action=${encodeURIComponent(action)}`
    })
        .then((response) => {
...
function rejectBorrow(bookId, action) {
    fetch("/approve-borrow", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: `book=${encodeURIComponent(bookId)}&action=${encodeURIComponent(action)}`
        ...
        .map((book) => {
            ...
            return `
                <article class="card book-card ${statusClass} ${selectedClass}">
                    ...
                    <p><strong>Category:</strong> ${book.category}</p>
                    ${reservationInfo}
                    <p><strong>Status:</strong> ${badgeText}</p>
                </article>
            `;
        })
        .join("");
...
function updateSelectedBooksStatus(newStatus, borrowerInfo = null) {
    const selectedIds = getSelectedBookIdsFromDom();
    if (selectedIds.length === 0) {
    ...
    ADMIN_STATE.books = ADMIN_STATE.books.map((book) => {
        if (selectedIds.includes(book.id) && book.status !== newStatus) {
            ...
            // Store borrower information if provided
            if (borrowerInfo && newStatus === "borrowed") {
                updatedBook.borrowedBy = borrowerInfo.username;
                updatedBook.borrowerName = borrowerInfo.fullName;
                updatedBook.borrowedDate = borrowerInfo.borrowedDate;
                updatedBook.returnDate = borrowerInfo.returnDate;
                booksToUpdate.push({
                    book: updatedBook,
                    borrowerInfo: borrowerInfo
                });
            } else if (newStatus === "available") {
                // Clear borrower info when marking as available
                delete updatedBook.borrowedBy;
                delete updatedBook.borrowerName;
                delete updatedBook.borrowedDate;
                delete updatedBook.returnDate;
            }
    ...
    // Send notifications to backend for borrowed books
    if (newStatus === "borrowed" && borrowerInfo) {
        booksToUpdate.forEach((item) => {
            const { book, borrowerInfo } = item;
            fetch("/mark-borrowed", {
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                },
                body: `username=${encodeURIComponent(borrowerInfo.username)}&bookId=${encodeURIComponent(book.id)}&bookTitle=${encodeURIComponent(book.title)}&returnDate=${encodeURIComponent(borrowerInfo.returnDate)}`
            })
                .then((response) => {
                    if (!response.ok) {
                        console.warn("Failed to send notification for", book.title);
                    }
                })
                .catch((error) => {
            ...
            if (currentAdminAction === "borrowed") {
                ...
                verifyRegisteredUser(username)
                    .then((exists) => {
                        ...
                        updateSelectedBooksStatus("borrowed", borrowerInfo);
...
function getCurrentUsername() {
    ...
    // Try to get from page (for admin dashboard showing name)
    const nameElement = document.querySelector(".app-bar span");
    if (nameElement && nameElement.textContent && nameElement.textContent.trim() !== "") {
        username = nameElement.textContent.trim();
        if (username !== "Campus Librarian") { // Don't use admin name
            return username;
        }
    }
...
function getBorrowedBooksForUser(username) {
    const books = ADMIN_STATE.books || [];
    return books.filter(book => book.status === "borrowed" && book.borrowedBy === username);
}

function renderBorrowedBooks() {
    const container = document.getElementById("book-inventory");
    const borrowedBooksSection = document.querySelector('[data-section="borrowed-books"]');
    
    // Handle user.html page with {{BORROWED_BOOKS}} placeholder
    if (!container && !borrowedBooksSection) {
        return; // Not on a user page
    }

    const username = getCurrentUsername();
    const borrowedBooks = getBorrowedBooksForUser(username);
        ...
        html = `
            <article class="card">
                <p style="text-align: center; color: #666; font-size: 14px;">
                    No borrowed books at this moment. Check back soon!
                </p>
            </article>
        `;
    } else {
        html = borrowedBooks.map(book => {
            const returnDate = book.returnDate ? new Date(book.returnDate).toLocaleDateString() : "Not specified";
            const today = new Date();
            const due = new Date(book.returnDate);
            const daysLeft = Math.ceil((due - today) / (1000 * 60 * 60 * 24));
            const isOverdue = daysLeft < 0;
            const isNearDue = daysLeft <= 2 && daysLeft >= 0;
            
            const statusClass = isOverdue ? "overdue" : isNearDue ? "near-due" : "";
            const statusText = isOverdue ? "OVERDUE" : isNearDue ? "DUE SOON" : daysLeft + " days left";
            
            return `
                <article class="card book-card borrowed-book-item ${statusClass}">
                    ...
                    <p><strong>Author:</strong> ${book.author}</p>
                    <p><strong>Category:</strong> ${book.category}</p>
                    <p><strong>Borrowed:</strong> ${book.borrowedDate || "Unknown"}</p>
                    <p><strong>Return by:</strong> <span class="return-date-highlight">${returnDate}</span></p>
    ...
    // Inject into the appropriate container
    if (container) {
...
function initUserPage() {
    ...
    // Display borrowed books on user page
    renderBorrowedBooks();
