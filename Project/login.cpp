...
static const std::string BORROWED_BOOKS_FILE = "borrowed_books.txt";
...
static std::map<std::string, std::map<std::string, std::pair<std::string, std::time_t>>> borrowedBooks;
...
std::string renderBrowsePage(const std::string &username) {
    ...
    for (const auto &entry : BOOK_TITLE_MAP) {
        ...
        bookGrid << "<article class=\"card book-card\">\n"
                 << "  <h3>" << title << "</h3>\n"
...
bool saveBorrowedBooks() {
    std::ofstream file(BORROWED_BOOKS_FILE, std::ios::trunc);
    if (!file) {
        return false;
    }
    for (const auto &userEntry : borrowedBooks) {
        const auto &username = userEntry.first;
        const auto &books = userEntry.second;
        for (const auto &bookEntry : books) {
            const auto &bookId = bookEntry.first;
            const auto &bookData = bookEntry.second;
            const auto &title = bookData.first;
            std::time_t returnTime = bookData.second;
            file << username << ":" << bookId << ":" << title << ":" << returnTime << "\n";
        }
    }
    return true;
}
...
void loadBorrowedBooks() {
    borrowedBooks.clear();
    std::ifstream file(BORROWED_BOOKS_FILE);
    if (!file) {
        return;
    }
    std::string line;
    while (std::getline(file, line)) {
        ...
        std::string bookId = parts[1];
        std::string title = parts[2];
        std::time_t returnTime = std::stoll(parts[3]);
        borrowedBooks[username][bookId] = std::make_pair(title, returnTime);
    }
}
...
std::string buildBorrowedBooksHtml(const std::string &username) {
    ...
    if (it == borrowedBooks.end() || it->second.empty()) {
    ...
    for (const auto &bookEntry : it->second) {
        const auto &bookId = bookEntry.first;
        const auto &bookData = bookEntry.second;
        const auto &title = bookData.first;
        std::time_t returnTime = bookData.second;
        
        std::tm *returnTimeStruct = std::localtime(&returnTime);
        char returnBuffer[32];
        std::strftime(returnBuffer, sizeof(returnBuffer), "%Y-%m-%d", returnTimeStruct);
        
        long secondsRemaining = static_cast<long>(returnTime - now);
        long daysRemaining = (secondsRemaining + 86399) / 86400; // Round up to next day
        ...
        if (secondsRemaining <= 0) {
            statusClass = "overdue-book";
            statusBadge = "OVERDUE";
            statusStyle = "background: rgba(239, 68, 68, 0.2); color: #ef4444;";
        } else if (daysRemaining <= 2) {
            statusBadge = std::to_string(daysRemaining) + " day" + (daysRemaining == 1 ? "" : "s") + " left";
            statusStyle = "background: rgba(249, 115, 22, 0.2); color: var(--danger);";
        } else {
            statusBadge = std::to_string(daysRemaining) + " days left";
            statusStyle = "background: rgba(34, 197, 94, 0.2); color: var(--success);";
        }
        
        html << "<article class=\"card book-card borrowed-book-item " << statusClass << "\">\n"
             << "  <div class=\"book-header\">\n"
             << "    <h3>" << title << "</h3>\n"
             << "    <span class=\"status-badge\" style=\"" << statusStyle << "\">" << statusBadge << "</span>\n"
             << "  </div>\n"
             << "  <p><strong>Book ID:</strong> " << bookId << "</p>\n"
             << "  <p><strong>Return by:</strong> <span class=\"return-date-highlight\">" << returnBuffer << "</span></p>\n"
             << "</article>\n";
    }
    return html.str();
}
...
int main() {
    ...
    while (true) {
        ...
        if (method == "GET" && path == "/logout") {
            ...
            headersOut["Location"] = "browse.html";
        ...
        if (method == "POST" && path == "/login") {
            ...
            auto isSafeRedirect = [&](const std::string &value) {
                static const std::vector<std::string> allowed = {"browse.html", "/browse.html", "user.html", "/user.html"};
            ...
            if (username == ADMIN_USER && password == ADMIN_PASS) {
                ...
                std::string bodyHtml = "<html><body><h1>Invalid login</h1><p>Username or password is incorrect.</p><p><a href='login.html'>Return to login</a></p></body></html>";
        ...
        if (method == "POST" && path == "/mark-borrowed") {
            auto form = parseFormData(body);
            std::string username = trim(form["username"]);
            std::string bookId = trim(form["bookId"]);
            std::string bookTitle = trim(form["bookTitle"]);
            std::string returnDateStr = trim(form["returnDate"]);
            
            if (!username.empty() && !bookId.empty() && !bookTitle.empty() && !returnDateStr.empty()) {
                // Parse the return date (format: YYYY-MM-DD)
                struct std::tm tm = {};
                sscanf(returnDateStr.c_str(), "%d-%d-%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday);
                tm.tm_year -= 1900;  // years since 1900
                tm.tm_mon -= 1;      // months are 0-11
                tm.tm_hour = 23;     // end of day
                tm.tm_min = 59;
                tm.tm_sec = 59;
                std::time_t returnTime = std::mktime(&tm);
                
                // Add borrowed book
                borrowedBooks[username][bookId] = std::make_pair(bookTitle, returnTime);
                saveBorrowedBooks();
                
                // Add notification to user
                std::string notification = "You have borrowed \"" + bookTitle + "\". Return by " + returnDateStr + ".";
                addUserNotification(username, notification);
                
                std::string responseJson = "{\"success\":true,\"message\":\"Book borrowed successfully\"}";
                sendResponse(clientSocket, "200 OK", { {"Content-Type", "application/json; charset=utf-8"} }, responseJson);
            } else {
                std::string responseJson = "{\"success\":false,\"message\":\"Missing required fields\"}";
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, responseJson);
            }
            closesocket(clientSocket);
            continue;
        }
