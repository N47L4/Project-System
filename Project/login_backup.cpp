    ...
    for (const auto &entry : bookReservations) {
        if (entry.second.first == username && entry.second.second > now) {
            ...
            html << "<article class=\"card book-card\">" 
                 ...
                 << "<button class=\"button-small\" type=\"button\" data-book-id=\"" << entry.first << "\" onclick=\"handleBorrowRequest('" << entry.first << "')\">Request Borrow</button>"
                 << "<button class=\"button-small danger\" type=\"button\" data-book-id=\"" << entry.first << "\" onclick=\"if(confirm('Cancel this reservation?')) { handleCancelReservation('" << entry.first << "'); }\">Cancel</button>"
                 << "</div>"
                 << "</article>\n";
        }
    }
...
int main() {
    ...
    while (true) {
        ...
        if (method == "POST" && path == "/borrow-request") {
            ...
            if (reservation == bookReservations.end() || reservation->second.first != username) {
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"error\":\"You do not have this book reserved.\"}");
                closesocket(clientSocket);
                continue;
            }
            // Add to pending borrows
            pendingBorrows[bookId] = username;
            addUserNotification(username, std::string("Borrow request submitted for book ") + bookId + ". Awaiting admin approval.");
            sendResponse(clientSocket, "200 OK", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"success\":true}");
            closesocket(clientSocket);
            continue;
        }

        if (method == "POST" && path == "/approve-borrow") {
            std::string adminUser = getUsernameForSession(cookieSession);
            if (adminUser != ADMIN_USER) {
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"error\":\"Admin access required.\"}");
                closesocket(clientSocket);
                continue;
            }
            auto form = parseFormData(body);
            std::string bookId = trim(form["book"]);
            std::string action = trim(form["action"]); // "approve" or "reject"
            if (bookId.empty()) {
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"error\":\"No book ID provided.\"}");
                closesocket(clientSocket);
                continue;
            }
            auto borrowRequest = pendingBorrows.find(bookId);
            if (borrowRequest == pendingBorrows.end()) {
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"error\":\"No pending borrow request found.\"}");
                closesocket(clientSocket);
                continue;
            }
            std::string borrowUsername = borrowRequest->second;
            if (action == "approve") {
                // Terminate the reservation (auto-borrowed)
                bookReservations.erase(bookId);
                addUserNotification(borrowUsername, std::string("Borrow approved for book ") + bookId + ". Book is now marked as borrowed.");
                sendResponse(clientSocket, "200 OK", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"success\":true}");
            } else if (action == "reject") {
                pendingBorrows.erase(bookId);
                addUserNotification(borrowUsername, std::string("Borrow request rejected for book ") + bookId + ".");
                sendResponse(clientSocket, "200 OK", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"success\":true}");
            } else {
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"error\":\"Invalid action.\"}");
            }
            pendingBorrows.erase(bookId);
            closesocket(clientSocket);
            continue;
        ...
        if (method == "GET" && path == "/pending-borrows") {
            std::string adminUser = getUsernameForSession(cookieSession);
            if (adminUser != ADMIN_USER) {
                sendResponse(clientSocket, "400 Bad Request", { {"Content-Type", "application/json; charset=utf-8"} }, "{\"error\":\"Admin access required.\"}");
                closesocket(clientSocket);
            ...
            json << "[";
            bool first = true;
            for (const auto &entry : pendingBorrows) {
                if (!first) json << ",";
                json << "{\"bookId\":\"" << jsonEscape(entry.first) << "\",\"username\":\"" << jsonEscape(entry.second) << "\"}";
                first = false;
            }
            json << "]";
            sendResponse(clientSocket, "200 OK", { {"Content-Type", "application/json; charset=utf-8"} }, json.str());
            closesocket(clientSocket);
            continue;
        }
